  ...
  if (window.jQuery && typeof jQuery.ajax === "function") {
    ...
    function fb3dFetch(url) {
      return new Promise(function (resolve, reject) {
        jQuery
          .ajax({ url: fb3dNormalizeUrl(url), dataType: "text" })
          .done(resolve)
          .fail(reject);
      });
    }
    FB3D_CLIENT_LOCALE.render = function () {
      ...
      window.PDFJS_LOCALE = {
        ...
        pdfJsWorker: fb3dNormalizeUrl(
          isStable ? pdfJs.stablePdfJsWorker : pdfJs.pdfJsWorker
        ),
      };
      Promise.all([
        fb3dFetch(
          FB3D_CLIENT_LOCALE.pluginurl +
            "assets/css/client.css?ver=" +
            FB3D_CLIENT_LOCALE.version
        ),
        fb3dFetch(
          FB3D_CLIENT_LOCALE.cacheurl +
            "skins.js?ver=" +
            FB3D_CLIENT_LOCALE.version
        ),
        ...
        jQuery("head").append(
          [
            ...
            fs[0].replace(
              /url\('..\//gi,
              "url('" +
                fb3dNormalizeUrl(FB3D_CLIENT_LOCALE.pluginurl + "assets/")
            ),
            "</style>",
          ].join("")
        );
